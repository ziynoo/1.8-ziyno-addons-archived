...
public class DeathTickTimer extends Gui {
    ...
    public void onRender(RenderGameOverlayEvent.Text e) {
        ...
        int redThresh   = Math.max(1, (int)Math.floor(pt * 5.0 / 20.0));   
        int yellowLow   = Math.max(redThresh + 1, (int)Math.floor(pt * 6.0 / 20.0)); 
        int yellowHigh  = Math.max(yellowLow,     (int)Math.floor(pt * 10.0 / 20.0)); 
        int greenLow    = Math.min(pt, yellowHigh + 1); 
