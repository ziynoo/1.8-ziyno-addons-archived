...
public class StormOneTickTimer extends Gui {
    ...
    // leap listen (no timeout)
    private static final String LEAP_NEEDLE = "leaped to ziyno!"; // case-insensitive
    ...
    public void onChat(ClientChatReceivedEvent event) {
        ...
        if (swRunning && isStormEnd) {
            ...
            guiShow = true;
            guiDisplayTicksRemaining = GUI_DISPLAY_TICKS; // show for ~1.5 seconds

            // ---- CHANGE #1: use (seconds - 35.0) for chat display ----
            ...
            // ---- CHANGE #2: new color thresholds on offsetSec ----
            EnumChatFormatting numColor;
            if (offsetSec <= 0.70) {
                numColor = EnumChatFormatting.GREEN;
            } else if (offsetSec <= 1.70) { // (0.70, 1.70]
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Text event) {
        if (!guiShow) return; // only show after Storm end lines
        if (mc.theWorld == null || mc.thePlayer == null) return;

        // ---- CHANGE #1 mirrored for GUI: (seconds - 35.0) ----
        ...
        // ---- CHANGE #2 mirrored for GUI thresholds ----
        // <= 0.70    : green
        // (0.70,1.70]: yellow
        // > 1.70      : red
        int color;
